...
static void	fill_res_lowerx(char *res, char *val_str, t_len *len, int val_len)
{
...
static int	minus_lowerx_dot(va_list *ap, t_len *len)
{
	unsigned int	val;
	char			*val_str;
	int				val_len;
	...
	{
		fill_res_lowerx(res, val_str, len, val_len);
		write(1, res, len->width);
	}
	free(res);
	free(val_str);
...
static int	minus_lowerx_nodot(va_list *ap, t_len *len)
{
	unsigned int	val;
	int				val_len;
	char			*res;
...
int	minus_lowerx(va_list *ap, t_len *len)
{
	...
	if (len->dot)
		rtn = minus_lowerx_dot(ap, len);
	else
		rtn = minus_lowerx_nodot(ap, len);
	return (rtn);
}
