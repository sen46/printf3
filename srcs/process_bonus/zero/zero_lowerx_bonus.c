...
static int	zero_lowerx_dot(va_list *ap, t_len *len)
{
	unsigned int	val;
	int				val_len;
	char			*res;
...
static int	zero_lowerx_nodot(va_list *ap, t_len *len)
{
	unsigned int	val;
	int				val_len;
	char			*res;
...
int	zero_lowerx(va_list *ap, t_len *len)
{
	...
	if (len->dot)
		rtn = zero_lowerx_dot(ap, len);
	else
		rtn = zero_lowerx_nodot(ap, len);
	return (rtn);
}
