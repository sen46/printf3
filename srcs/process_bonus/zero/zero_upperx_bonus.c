...
static void	zero_uppers_dot_sub(char *res, char *val_str, t_len *len)
{
	int	val_len;
...
static int	zero_upperx_dot(va_list *ap, t_len *len)
{
	unsigned int	val;
	int				val_len;
	char			*res;
	...
	if (val == 0 && len->precision == 0)
		...
		zero_uppers_dot_sub(res, val_str, len);
	free(res);
	free(val_str);
	return (len->width);
}

static int	zero_upperx_nodot(va_list *ap, t_len *len)
{
	unsigned int	val;
	int				val_len;
	char			*res;
...
int	zero_upperx(va_list *ap, t_len *len)
{
	...
	if (len->dot)
		rtn = zero_upperx_dot(ap, len);
	else
		rtn = zero_upperx_nodot(ap, len);
	return (rtn);
}
